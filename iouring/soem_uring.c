...
int iouring_init() {
  ...
  printf("[INFO] io_uring_queue_init OK\n");
  ...
  ret = io_uring_queue_init_params(IOURING_QDEPTH, &ioctx.ring, &params);
  if (ret < 0) {
  ...
  printf("[INFO] io_uring_queue_init_params OK: features=0x%x, wq_fd=%d, sq_thread_cpu=%d\n",
         params.features, params.wq_fd, params.sq_thread_cpu);

  printf("flags after setup = 0x%x\n", params.flags);
